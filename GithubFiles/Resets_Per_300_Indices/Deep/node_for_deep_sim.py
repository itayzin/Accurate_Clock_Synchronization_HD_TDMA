...
class node():
    ...
    def train_node(self, first_iter_flag):
        ...
        # self.DNN.Phase_DNN  = self.DNN.min_Phase_DNN
        # self.DNN.Period_DNN = self.DNN.min_Period_DNN
        ...
        if np.mod(k, self.safedict["N"] - 1 + 1) == self.safedict["N"] - 1:
            ...
            with torch.no_grad():
                ...
                Phase_Alpha = self.DNN.Phase_DNN(torch.cat((torch.tensor(
                    self.Time_Diff) + self.Propagation_Vector, self.CData[:, 0]), dim=0))
            ...
            with torch.no_grad():
                Updating = False
                if k > 2 * nns.N:
                    self.Period[k + 1] = self.Period[k] +  self.Period_Update / nns.N # Bad, needs eps_0 fix!
                else:
                    self.Period[k + 1] = self.Period[k]
                self.Period_Ob[Transmitter] = (
                    Network_Times_Vector[Transmitter] - self.Network_Times_Vector[
    ...
    def Time_Step_Fake(self, k, Alpha_Vector, Network_Times_Vector, Propagation_Vector, Period_Alpha_Vector):
        Transmitter = np.mod(k, self.safedict["N"])
        if np.mod((k) // (nns.N), 3) == 0:
            peps_0 = nns.PERIOD_EPS_0
            eps_0 = 0  # nns.EPS_0
            ...
            eps_0 = 0  # nns.EPS_0
        if np.mod(k, self.safedict["N"] - 1 + 1) == self.safedict["N"] - 1:
            self.Period_Ob_Fake[Transmitter] = (
                Network_Times_Vector[Transmitter] - self.Network_Times_Vector_Fake[
                    Transmitter]) / nns.N


            self.Network_Times_Vector_Fake[np.mod(
                k, self.safedict["N"] - 1 + 1)] = Network_Times_Vector[np.mod(k, self.safedict["N"])]
            self.Time_Diff_Fake[np.mod(k, self.safedict["N"] - 1 + 1)] = self.Network_Times_Vector_Fake[np.mod(
                k, self.safedict["N"] - 1 + 1)] - self.Time_Fake[k]
            if k > nns.N:
                Alpha_Vector = self.DNN.Phase_DNN(
                    torch.cat(((torch.Tensor(
                    self.Time_Diff_Fake).double() + Propagation_Vector.double()), self.CData[:, k]), dim=0))
                Period_Alpha_Vector = self.DNN.Period_DNN(
                    torch.cat(((torch.Tensor(self.Period_Ob_Fake) - self.Period_Fake[k - nns.N]), self.CData[:, k]), dim=0))
                self.Period_Fake_Update = (peps_0 * torch.dot(
                    Period_Alpha_Vector, (torch.Tensor(self.Period_Ob_Fake) - self.Period_Fake[k - nns.N])))
                self.Period_Fake[k + 1] = self.Period_Fake[k] + self.Period_Fake_Update / nns.N
            else:
                if k >  nns.N:
                    self.Period_Fake[k + 1] = self.Period_Fake[k] + self.Period_Fake_Update / nns.N
                else:
                    self.Period_Fake[k + 1] = self.Period_Fake[k]

            self.Time_Fake[k + 1] = self.Time_Fake[k] + self.Period_Fake[k] + eps_0 * (
                torch.dot(torch.Tensor(Alpha_Vector).double(), (torch.Tensor(
                    self.Time_Diff_Fake).double() + Propagation_Vector.double())))  
        else:
            self.Period_Ob_Fake[Transmitter] = (
                Network_Times_Vector[Transmitter] - self.Network_Times_Vector_Fake[
                    Transmitter]) / nns.N
            ...
            if k > 2 * nns.N:
                ...
                self.Period_Fake[k + 1] = self.Period_Fake[k]
            self.Time_Fake[k + 1] = self.Time_Fake[k] + self.Period_Fake[k]
...
class CreateSimpleDnn():
    ...
    def Train(self, Data, CData, Data_Diff, save_log=False):
        ...
        for epoch in range(1):
            ...
            if nns.EPOCHPLOT:
                plt.subplot(411)
                plt.plot([np.mod(self.Node.Time_Fake[k].clone().detach().numpy(
                ), self.Node.Period_Fake[k].clone().detach().numpy()) for k in range(len(self.Node.Time_Fake))])
                plt.title(
                    f"Phases of node {self.Node.ID} at epoch {self.Epoch}")
                plt.subplot(412)
                plt.plot([self.Node.Period_Fake[k].clone().detach().numpy()
                          for k in range(len(self.Node.Period_Fake))])
                plt.title(
            ...
            if ("Period" in nns.LEARNER_LIST) and ("Phase" in nns.LEARNER_LIST):
                ...
                self.optimizer_Period.zero_grad()
                batch_loss.backward(retain_graph=True)  # Retain the computation graph for the next backward pass
                # Zero out gradients for Phase_DNN parameters to effectively freeze the learning
                for param in self.Phase_DNN.parameters():
                    if param.grad is not None:
                        param.grad.zero_()
                # torch.nn.utils.clip_grad_norm_(self.Period_DNN.parameters(), max_norm=1)
                self.optimizer_Period.step()
            elif ("Phase" in nns.LEARNER_LIST):
                # Freeze learning for Period_DNN
                self.optimizer_Phase.zero_grad()
                ...
                for param in self.Period_DNN.parameters():
                    if param.grad is not None:
                        param.grad.zero_()
                # torch.nn.utils.clip_grad_norm_(self.Phase_DNN.parameters(), max_norm=1)
                self.optimizer_Phase.step()
            else:
    ...
    def loss_func_phase(self, Data):
        ...
        for batch in range(nns.N, self.batches):
            ...
            if self.Node.Connectivity_Vector[transmitter] == 1:
                ...
                instantenous_loss = nns.loss_phase_element((self.Node.Time_Fake[batch] - Data[transmitter, batch]), batch)
    ...
    def loss_func_period(self, Data):
        ...
        for batch in range(2 * nns.N, self.batches):
            ...
            transmitter = np.mod(batch, self.Node.safedict["N"])
            if self.Node.Connectivity_Vector[transmitter] == 1:
                instantenous_loss = nns.loss_period_element((self.Node.Period_Fake[batch - nns.N] - (Data[transmitter, batch] - Data[transmitter, batch - nns.N]) / nns.N), batch)
...
class Weighted_Averaging_Learner(nn.Module):
    ...
    def forward(self, x):
        # interm_weights = NormalizationLayer()(self.weight)
