...
import sys
from node_for_analytic_sim import node
...
import random
def multiformation_plot(Node_Aggregation, SIM_LEN, network_type = "Random",start = 0 , end = 4000, which_nodes = None):
    if which_nodes == None:
        which_nodes = [k for k in range(nns.N)]
    plt.figure()
    plt.subplot(221)
    Node_Aggregation.plot_phases(end, start = start, which_nodes=which_nodes)
    plt.title(f"Phases of {network_type} Network")
    plt.legend(which_nodes)

    plt.subplot(222)
    Node_Aggregation.plot_periods(end, start = start, which_nodes=which_nodes)
    plt.title(f"Periods of {network_type} Network")
    plt.legend(which_nodes)

    plt.subplot(223)
    Node_Aggregation.plot_periods_skew(end, which_nodes=which_nodes)
    plt.title(f"Periods minus Mean Period of {network_type} Network")
    plt.legend(which_nodes)

    plt.subplot(224)
    Node_Aggregation.plot_periods_average(end, which_nodes=which_nodes)
...
def reset_nodes(Node_Aggregation, k):
    fa = F_Nom * (1 - 10 ** (-6) * nns.PPM_VAR)
    fb = F_Nom * (1 + 10 ** (-6) * nns.PPM_VAR)
    New_Periods = 1. / F_lambda(fa, fb, nns.N)
    Phase_Shifts =  New_Periods - nns.Initial_Time_Scaling * np.random.rand(
        nns.N, 1) * New_Periods
    pick_amount = int(np.floor(nns.reset_nodes_pick_ratio * nns.N))
    indices = [k for k in range(nns.N)]
    random.shuffle(indices)
    for pick_index in range(pick_amount):
        node_index = indices[pick_index]
        Node_Aggregation.node_array[node_index].Time[k] += Phase_Shifts[node_index]
        Node_Aggregation.node_array[node_index].Period[k] *= 0
        Node_Aggregation.node_array[node_index].Period[k] += New_Periods[node_index]
...
def Temporary_Dictionary_Preparation(load_path=False):
    ...
    sim_properties_dict["layer_mode"] = "min_max_avg"
...
class node_aggregation():
    def __init__(self, sim_properties_dict):
        ...
        self.prop_delay_mat = torch.zeros(
            sim_properties_dict["N"], sim_properties_dict["N"])
    def plot_losses_adv(self, type, which_nodes = None):
        if which_nodes == None:
            which_nodes = [k for k in range(nns.N)]
        losses = []
        if type == "Phases":
            for k in which_nodes:
                losses = Node_Aggregation.node_array[k].loss_phase_vec
                plt.semilogy(np.linspace(nns.N, len(Node_Aggregation.node_array[k].Time), len(losses)), losses, label=f"node {k}")
            plt.legend()
            plt.title("Phase Losses")
            plt.show()
        else:
            for k in range(nns.N):
                Node_Aggregation.node_array[k].DNN.loss_func_period(Node_Aggregation.node_array[k].Data)
                losses.append(Node_Aggregation.node_array[k].DNN.loss_int_vec_period)
                plt.plot(np.linspace(nns.N, len(Node_Aggregation.node_array[k].Period), len(losses[k])), losses[k], label=f"node {k}")
            plt.legend()
            plt.title("Period Losses")
            plt.show()
    ...
    def calc_prop_delay(self, node_A, node_B, precalculated = True):
        ID1 = node_A.id
        ID2 = node_B.id
        if precalculated:
    ...
    def prepare_data(self):
        for node_indx in range(self.safedict["N"]):
            ...
            self.node_array[node_indx].loss_phase_vec = []

        for batch in range(self.safedict["sim_length"] - 1):
            if (batch >= nns.network_reset_index) and (np.mod(batch - nns.network_reset_index, nns.reset_interval) == 0):
                reset_nodes(self, batch)
        ...
        for batch in range(batches):
            ...
            for j in range(self.safedict["N"]):
                if Deep_Enabled and (Transmitter == self.safedict["N"] - 1):
                    Network_Times_Diff_Vec[j, Transmitter] = Network_Times_Vector[Transmitter] - self.node_array[j].Time[batch]
                    Phase_Alpha = self.node_array[j].DNN.Phase_DNN(torch.cat(
                        (Network_Times_Diff_Vec[j, :] + self.node_array[j].Propagation_Vector,
    ...
    def start_training_recursive_root(self, training_node_indices, first_iter_flag):
        with concurrent.futures.ProcessPoolExecutor() as executor:
           self.futures = [executor.submit(self.node_array[i].train_node, first_iter_flag) for i in training_node_indices]
           concurrent.futures.wait(self.futures)  
    ...
    def start_training_recursive(self):
        ...
        training_node_indices = [i for i in range(nns.N)] # Init training nodes
        counts = 0
        max_counts = 1
        while len(training_node_indices) != 0 and max_counts > counts:
            training_node_indices = self.start_training_recursive_root(training_node_indices, first_iter_flag) # ONLY THOSE WHO FAILED
            first_iter_flag = False
            counts += 1
            self.prepare_data()
            for i in range(nns.N):
                pass
        self.failed_node_indices = training_node_indices

    def plot_periods(self, end, lw = 1, start = 1, which_nodes = None):
        if which_nodes == None:
            which_nodes = [k for k in range(nns.N)]
        for i in which_nodes:
            plt.plot(np.arange(start,end), self.node_array[i].Period.detach().numpy()[start:end], lw=lw)
        pass

    def plot_periods_skew(self, end, which_nodes = None):
        if which_nodes == None:
            which_nodes = [k for k in range(nns.N)]
        ...
        pass
        plt.plot((periods[which_nodes, :] - np.mean(periods, axis=0)).T)

    def plot_periods_average(self, end, which_nodes = None):
        if which_nodes == None:
            which_nodes = [k for k in range(nns.N)]
        periods = np.zeros((nns.N, end))
        for i in range(self.safedict["N"]):
    ...
    def plot_phases(self, end, start = 0, which_nodes = None):
        if which_nodes == None:
            which_nodes = [k for k in range(nns.N)]
        nns.N = self.safedict["N"]
        Times = np.zeros((nns.N, end-start))
        for i in range(self.safedict["N"]):
            Times[i, :] = self.node_array[i].Time[start:end].detach().numpy()
        plt.plot(np.arange(start,end), (Times[which_nodes, :] - np.mean(Times, 0)).T)
    ...
    def plot_connection(self):
        strength_matrix_period = np.array(
            [self.node_array[i].period_connectivity_history.detach().numpy() for i in range(nns.N)])
        strength_matrix_phase = np.array(
            [self.node_array[i].phase_connectivity_history.detach().numpy() for i in range(nns.N)])

        con_mat = self.connection_mat.detach().numpy()
        locations = np.array(self.safedict["location"])

        fig, axs = plt.subplots(1, 2, figsize=(20, 10))

        c_map = LinearSegmentedColormap.from_list('mycmap', ['b', 'r'])

        for ax, strength_matrix, title in zip(axs, [strength_matrix_period, strength_matrix_phase],
                                              ['Period Strength', 'Phase Strength']):
            ax.set_title(title)
            ax.set_axis_on()
            ax.grid('on')

            for ID1 in range(self.safedict["N"]):
                for ID2 in range(self.safedict["N"]):
                    if con_mat[ID1, ID2] == 1:
                        # Get the positions of the two nodes
                        x1, y1 = locations[ID1]
                        x2, y2 = locations[ID2]

                        # Compute the strength and plot the line with gradient
                        strength = strength_matrix[ID1, ID2]
                        points = [[x1, y1], [x2, y2]]
                        lc = LineCollection([points], linewidths=strength,
                                            colors=c_map(np.linspace(0, 1, 2)), alpha=strength)
                        ax.add_collection(lc)
                        # Plot an arrow at the end of the line
                        ax.annotate("", xy=(x2, y2), xytext=(x1, y1),
                                    arrowprops=dict(arrowstyle="->", lw=strength, color='r', alpha=strength))

                # Plot the nodes
                ax.scatter(*locations[ID1], color='k', label=str(ID1))
...
if __name__ == '__main__':
    ...
    nns.S_LENGTH = int(nns.S_LENGTH)
    np.random.seed(int(nns.SEED_ID))
    ...
    if nns.RANDOMIZE_INITS:
        ...
        safedict["Initial_Time"] = np.random.rand(
    ...
    if nns.RANDOMIZE_INITS:
        ...
        safedict["Initial_Period"] = np.array(safedict["Initial_Period"])
        safedict["Initial_Time"] = np.array(safedict["Initial_Time"])
    if nns.LOAD_SAFEDICT:
        safedict = load_data(nns.SEED_ID, safedict)
        q_mat, simeone_alphas, con_graph = get_q_alphas_connectivity(
            safedict["location"], safedict["Power_Exponent"], safedict["Power_Threshold"], safedict["TX_Power"][0],
            safedict["Speed_Of_Light"])
        safedict["alpha_mat"] = alpha_mat_random
    ...
    Node_Aggregation.set_mode(safedict["layer_mode"])
    ...
    logcell = {
        ...
        "Failed_Nodes" : list()}
    SIM_LEN = safedict["sim_length"] - 2
    modes = ["average_over_connections", "min_max_avg_neutralize"]

    N_ = {}
    NPDS_ = {}
    for k in range(len(modes)):
        Node_Aggregation.set_mode(modes[k])
        Node_Aggregation.prepare_data()
        Node_Aggregation.save_times_periods(modes[k])
